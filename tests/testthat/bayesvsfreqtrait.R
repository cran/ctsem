      ...
      for(nsubjects in c(5,20)){
        for(tpoints in c(6,25)){
          ...
          genm=ctModel(Tpoints=tpoints,
            ...
            MANIFESTTRAITVAR=matrix(c(1,.5,0,0,2,0,0,0,.02),nmanifest,nmanifest),
            MANIFESTMEANS=matrix(c(-5,2,1), nrow=nmanifest, ncol=1))
          ...
          tc=proc.time()[3]
          se1=fit$mxobj$output$standardErrors[rownames(fit$mxobj$output$standardErrors)==pars[1,pari]]
          ...
          ta=proc.time()[3]
          sfit=ctStanFit(datalong=ldat, ctstanmodel=sm,iter=300,chains=2,esthyper = F,optimize=F,kalman=TRUE)
          tb=proc.time()[3]
